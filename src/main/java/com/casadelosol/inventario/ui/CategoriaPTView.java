...
public class CategoriaPTView implements Refreshable {

    private final TreeView<CategoriaPT> treeView = new TreeView<>();
    private final CategoriaPTDAO dao = new CategoriaPTDAO();

    private TreeItem<CategoriaPT> rootItem;

    public Node getView() {
        VBox root = new VBox(15);
        root.setPadding(new Insets(20));

        Label title = new Label("Categorías - Productos Terminados");
        ...
        treeView.setCellFactory(tv -> new TextFieldTreeCell<>(new CategoriaPTStringConverter()));
    ...
    private void showAddDialog() {
        Dialog<CategoriaPT> dialog = new Dialog<>();
        dialog.setTitle("Nueva Categoría");
        dialog.setHeaderText("Agregar categoría de producto terminado");
        ...
        ComboBox<CategoriaPT> cbParent = new ComboBox<>();
        ...
        dialog.setResultConverter(btn -> {
            if (btn == btnGuardar) {
                CategoriaPT cat = new CategoriaPT(tfNombre.getText().trim());
    ...
    private void showEditDialog() {
        TreeItem<CategoriaPT> selected = treeView.getSelectionModel().getSelectedItem();
        if (selected == null) {
            showAlert("Seleccione una categoría para editar");
            return;
        }

        CategoriaPT cat = selected.getValue();
        Dialog<CategoriaPT> dialog = new Dialog<>();
        ...
        ComboBox<CategoriaPT> cbParent = new ComboBox<>();
        ObservableList<CategoriaPT> cats = FXCollections.observableArrayList(dao.findAll());
        cats.remove(cat);
    ...
    @Override
    public void refresh() {
        rootItem = new TreeItem<>(new CategoriaPT("Raíz"));
        rootItem.setExpanded(true);

        Map<Integer, TreeItem<CategoriaPT>> itemMap = new HashMap<>();
        for (CategoriaPT cat : dao.findAll()) {
            itemMap.put(cat.getId(), new TreeItem<>(cat));
        }

        for (CategoriaPT cat : dao.findAll()) {
            TreeItem<CategoriaPT> item = itemMap.get(cat.getId());
    ...
    private static class CategoriaPTStringConverter extends javafx.util.StringConverter<CategoriaPT> {
        @Override
        public String toString(CategoriaPT object) {
            return object != null ? object.getNombre() : "";
        }

        @Override
        public CategoriaPT fromString(String string) {
            return new CategoriaPT(string);
        }
    }
}
